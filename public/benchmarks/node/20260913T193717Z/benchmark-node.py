...
def main() -> None:
    ...
    result = {
        ...
        'method': {
            ...
            'dependencies': 'Installed npm package instances minus the root SDK. Includes transitive, peer, and optional packages actually installed on this platform; nested duplicate installations count separately. Development dependencies are omitted.',
            'startup': 'Median process.hrtime.bigint time around await import(package), using the public root ESM entry in a fresh Node process. Two unmeasured warmups populate OS file caches. Excludes process launch, shutdown, client construction, and provider calls. Persistent Node compile caching is explicitly disabled.',
    ...
    for key, label, package in [('baseline', 'Empty Node', ''), *CLIENTS]:
        ...
        if args.lock_dir and package:
            ...
            command = ['npm', 'install', '--save-exact', 'file:lm15.tgz' if key == 'lm15' else package]
        if package:
        ...
        statement = f'await import({json.dumps(package)});' if package else 'void 0;'
