...
def main():
    ...
    source_pin=work/'source-pins.json'
    if source_pin.exists():
        if json.loads(source_pin.read_text()) != sources: raise RuntimeError('Source snapshots changed; use a fresh work directory')
    elif (work/'sources').exists():
        raise RuntimeError('Existing work directory has no source fingerprint; use a fresh work directory')
    else:
        source_pin.write_text(json.dumps(sources,indent=2)+'\n')
