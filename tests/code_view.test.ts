...
import { EXAMPLE_INPUTS, EXAMPLE_INSTRUCTIONS, EXAMPLE_SPEC, judgeGo, judgeJavascript, judgePython, judgeRust, type JudgeSpec } from "../src/playground/judge.ts";
...
test("every language marks the person's values, LM15's calls and the plumbing, in Chat and in Judge", () => {
  const fields: JudgeSpec = { ...EXAMPLE_SPEC, shape: "fields", instructions: EXAMPLE_INSTRUCTIONS, fields: [{ name: "note", type: "text" }, { name: "price_eur", type: "number" }] };
  ...
  for (const render of judge) {
    for (const [spec, input] of [[EXAMPLE_SPEC, EXAMPLE_INPUTS[0]!], [fields, { note: "A note.", price_eur: 48 }], [conversation, turns]] as const) {
      for (const connection of [typesafe, openai]) {
        ...
        for (const text of ["How good is this wine, according to the note?", "Faulty or unpleasant", "faulty", "Fruit-forward", "mineral", "Does the note say the wine will improve with age?"]) assert.ok(values.includes(text), `${render.name}: ${text} is a value`);
        ...
        if (spec.instructions) assert.ok(values.includes(spec.instructions), `${render.name}: the instructions are a value`);
  ...
  const hostile = judgeJavascript(openai, { ...EXAMPLE_SPEC, instructions: "a\u0001b\u0005c" }, ["x\u0002y"]);
  assert.match(hostile.text, /a\\u0001b\\u0005c/);
  assert.match(hostile.text, /x\\u0002y/);
});
