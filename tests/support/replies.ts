...
export function jevAnswers(keys?: readonly string[]): Record<string, unknown> {
  const all: Record<string, unknown> = { quality: { type: "score", probabilities: { "0": 0, "1": 0, "2": 0.1, "3": 0.8, "4": 0.1 } }, style: { type: "choice", choice: "fruit", probabilities: { fruit: 0.9, oak: 0.1, mineral: 0 } }, ageing: { type: "noul", noul: 0.97 } };
...
export function judgedText(keys?: readonly string[]): string {
  const all = JSON.parse(JUDGED_TEXT) as Record<string, unknown>;
